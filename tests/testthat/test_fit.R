...
test_that('fitting works for PL', {
  set.seed(888)
  params <- c(K = 1.3, c = 1, theta = 1, N = 100)
  sims <- generate_hawkes_event_series(params = params, model_type = 'PL')
  fitted <- fit_series(data = list(sims), model_type = 'PL', observation_time = Inf)
  expect_equal(fitted$convergence, 0)
  expect_equal(nrow(fitted$data[[1]]), nrow(sims))
})
